...
class CashPaymentManager {
    ...
    constructor() {
        ...
        this.changeAmountInput = document.getElementById('change-amount');
        this.currentInputString = '';
    ...
    initializeInputFields() {
        this.inputFields.forEach((input) => {
            ...
            input.addEventListener('input', () => this.handleKeyboardInput(input));
        });
        if (this.inputFields.length > 0) {
            this.setActiveInput(this.inputFields[0]);
        }
    }
    handleKeyboardInput(input) {
        if (this.activeInput === input) {
            ...
            if (newValue === '' || newValue === '0') {
                ...
                this.currentInputString = newValue;
                input.value = this.formatAmount(this.parseAmount(newValue));
            }
        }
    }
    setActiveInput(input) {
        ...
        this.activeInput.classList.add('active');
        const currentValue = this.activeInput.value.replace(/[^0-9]/g, '');
        this.currentInputString = (currentValue === '0' || currentValue === '') ? '' : currentValue;
    ...
    handleKeyInput(key) {
        ...
        if (this.currentInputString.length > 10) {
            this.currentInputString = this.currentInputString.substring(0, 10);
        }
    ...
    updateActiveDisplay() {
        if (this.activeInput) {
            this.activeInput.value = this.currentInputString;
        }
    }
    ...
    handleSubmit() {
        ...
        console.log("--- データ送信が実行されました ---");
        this.sendDataToFirebase(Number(purchaseAmount), Number(paidAmount), Number(changeAmount.replace(/,/g, '')));
        console.log(`支払い金額: ${purchaseAmount}`);
        console.log(`投入金額: ${paidAmount}`);
        console.log(`おつり: ${changeAmount}`);
        console.log("----------------------------------");
        this.flashSubmitButtonColor();
    }
    flashSubmitButtonColor() {
        const duration = 2000;
        this.submitButton.style.backgroundColor = this.FEEDBACK_COLOR;
        setTimeout(() => {
            this.submitButton.style.backgroundColor = this.DEFAULT_SUBMIT_COLOR;
        }, duration);
    }
    sendDataToFirebase(purchaseAmount, paidAmount, changeAmount) {
...
document.addEventListener('DOMContentLoaded', () => {
    new CashPaymentManager();
});
//# sourceMappingURL=aeonControlePanel.js.map